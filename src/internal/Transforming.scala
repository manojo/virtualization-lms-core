...
trait Transforming extends Expressions with Blocks with OverloadHack {
  ...
  def ntype[A, B](m: Nul[A]): Nul[B] = m.asInstanceOf[Nul[B]] // hack: need to pass explicit manifest during mirroring
  ...
  def mirror[A: Typ: Nul](e: Def[A], f: Transformer)(implicit pos: SourceContext): Exp[A] = mirrorDef(e,f)

  def mirrorDef[A: Typ: Nul](e: Def[A], f: Transformer)(implicit pos: SourceContext): Def[A] = sys.error("don't know how to mirror " + e)

  def mirrorFatDef[A: Typ: Nul](e: Def[A], f: Transformer)(implicit pos: SourceContext): Def[A] = sys.error("don't know how to mirror " + e) //hm...
...
trait FatTransforming extends Transforming with FatExpressions {
  ...
  override def mirror[A: Typ: Nul](e: Def[A], f: Transformer)(implicit pos: SourceContext): Exp[A] = (e match {
    case Forward(x) => toAtom(Forward(f(x)))(mtype(typ[A]), ntype(nul[A]), pos)
    case _ => super.mirror(e,f)
  }).asInstanceOf[Exp[A]]

  //def mirror[A: Typ: Nul](e: FatDef, f: Transformer): Exp[A] = sys.error("don't know how to mirror " + e)
