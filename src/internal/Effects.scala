...
*/
  ...
  */
  ...
  protected override implicit def toAtom[T: Typ: Nul](d: Def[T])(implicit pos: SourceContext): Exp[T] = {
/*
...
*/
  ...
  def reflectMirrored[A: Typ: Nul](zd: Reflect[A])(implicit pos: SourceContext): Exp[A] = {
    checkContext()
    // warn if type is Any. TODO: make optional, sometimes Exp[Any] is fine
  ...
  def reflectMutable[A: Typ: Nul](d: Def[A])(implicit pos: SourceContext): Exp[A] = {
  ...
  def reflectWrite[A: Typ: Nul](write0: Exp[Any]*)(d: Def[A])(implicit pos: SourceContext): Exp[A] = {
  ...
  def reflectEffect[A: Typ: Nul](x: Def[A])(implicit pos: SourceContext): Exp[A] = reflectEffect(x, Simple()) // simple effect (serialized with respect to other simples)

  def reflectEffect[A: Typ: Nul](d: Def[A], u: Summary)(implicit pos: SourceContext): Exp[A] = {
  ...
  def reflectEffectInternal[A: Typ: Nul](x: Def[A], u: Summary)(implicit pos: SourceContext): Exp[A] = {
...
//        findDefinition(zd) map (_.sym) filter (context contains _) getOrElse { // local cse TODO: turn around and look at context first??
  ...
  def reifyEffects[A: Typ: Nul](block: => Exp[A], controlScope: Boolean = false): Block[A] = {
    val save = context
    context = Nil
  ...
  def reifyEffectsHere[A: Typ: Nul](block: => Exp[A], controlScope: Boolean = false): Block[A] = {
    val save = context
    if (save eq null)
      context = Nil
