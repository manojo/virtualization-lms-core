...
/*
  object SynchronizedArrayBuffer {
    def apply[A: Typ: Nul](xs: Rep[A]*)(implicit pos: SourceContext) = arraybuffer_new(xs)
  }
*/
...
trait SynchronizedArrayBufferOpsExp extends SynchronizedArrayBufferOps with ArrayBufferOpsExp {
  case class SyncArrayBufferNew[A: Typ: Nul](xs: Seq[Exp[A]]) extends Def[ArrayBuffer[A]]  {
    val mA = manifest[A]
  }

  // all array buffers are synchronized (nackward compat). TODO: separate constructor

  override def arraybuffer_new[A: Typ: Nul](xs: Seq[Exp[A]])(implicit pos: SourceContext) = reflectMutable(SyncArrayBufferNew(xs))
}
