...
trait StructOps extends Base {
  ...
  def __new[T: Typ: Nul](args: (String, Boolean, Rep[T] => Rep[_])*): Rep[T] = record_new(args)

  class RecordOps(record: Rep[Record]) {
    def selectDynamic[T: Typ: Nul](field: String): Rep[T] = record_select[T](record, field)
  }
  implicit def recordToRecordOps(record: Rep[Record]) = new RecordOps(record)

  def record_new[T: Typ: Nul](fields: Seq[(String, Boolean, Rep[T] => Rep[_])]): Rep[T]
  def record_select[T: Typ: Nul](record: Rep[Record], field: String): Rep[T]
  def field[T: Typ: Nul](struct: Rep[Any], index: String)(implicit pos: SourceContext): Rep[T]
}
...
trait StructExp extends StructOps with StructTags with BaseExp with EffectExp with VariablesExp /*with ObjectOpsExp*/ with StringOpsExp with OverloadHack {
  ...
  /* override def fresh[T: Typ: Nul] = manifest[T] match {
  ...
  case class FieldUpdate[T: Typ: Nul](struct: Exp[Any], index: String, rhs: Exp[T]) extends Def[Unit] {
    def mA = manifest[T]
    def nA = nul[T]
  }

  def struct[T: Typ: Nul](tag: StructTag[T], elems: (String, Rep[Any])*)(implicit o: Overloaded1, pos: SourceContext): Rep[T] = struct[T](tag, elems)
  def struct[T: Typ: Nul](tag: StructTag[T], elems: Seq[(String, Rep[Any])])(implicit pos: SourceContext): Rep[T] = SimpleStruct(tag, elems)

  def field[T: Typ: Nul](struct: Rep[Any], index: String)(implicit pos: SourceContext): Rep[T] = FieldApply[T](struct, index)
  def var_field[T: Typ: Nul](struct: Rep[Any], index: String)(implicit pos: SourceContext): Var[T] = Variable(FieldApply[Var[T]](struct, index))
  def field_update[T: Typ: Nul](struct: Exp[Any], index: String, rhs: Exp[T]): Exp[Unit] = reflectWrite(struct)(FieldUpdate(struct, index, rhs))

  def record_new[T: Typ: Nul](fields: Seq[(String, Boolean, Rep[T] => Rep[_])]) = {
    ...
    val fieldSyms = fields map {
      ...
      case (index, true, rhs) => val y = rhs(x); (index, var_new(y)(y.tp, y.nl, implicitly[SourceContext]).e)
    }
    val ManifestTyp(manifest) = typ[T]
    struct(AnonTag(manifest.asInstanceOf[RefinedManifest[T]]), fieldSyms)
  }

  def record_select[T: Typ: Nul](record: Rep[Record], fieldName: String) = {
  ...
  //override def object_unsafe_immutable[A: Typ: Nul](lhs: Exp[A])(implicit pos: SourceContext) = lhs match {
  ...
  override def mirror[A: Typ: Nul](e: Def[A], f: Transformer)(implicit pos: SourceContext): Exp[A] = (e match {
    case SimpleStruct(tag, elems) => struct(tag, elems map { case (k,v) => (k, f(v)) })(mtype(manifest[A]), ntype(nul[A]), pos)
    case FieldApply(struct, key) => field(f(struct), key)(mtype(manifest[A]), ntype(nul[A]), pos)
    case Reflect(FieldApply(struct, key), u, es) => reflectMirrored(Reflect(FieldApply(f(struct), key), mapOver(f,u), f(es)))(mtype(manifest[A]), ntype(nul[A]),  pos)
    case Reflect(e @ FieldUpdate(struct, key, rhs), u, es) =>
      reflectMirrored(Reflect(FieldUpdate(f(struct), key, f(rhs))(e.mA, e.nA), mapOver(f,u), f(es)))(mtype(manifest[A]), ntype(nul[A]),  pos)
    case Reflect(SimpleStruct(tag, elems), u, es) => reflectMirrored(Reflect(SimpleStruct(tag, elems map { case (k,v) => (k, f(v)) }), mapOver(f,u), f(es)))(mtype(manifest[A]), ntype(nul[A]),  pos)
  ...
  def classTag[T: Typ: Nul] = ClassTag[T](structName(manifest[T]))
...
trait StructExpOpt extends StructExp {
  ...
  override def field[T: Typ: Nul](struct: Exp[Any], index: String)(implicit pos: SourceContext): Exp[T] = fieldLookup[T](struct, index) match {
  ...
  /* override def var_field[T: Typ: Nul](struct: Exp[Any], index: String)(implicit pos: SourceContext): Var[T] = fieldLookup(struct, index) match {
...
trait StructExpOptCommon extends StructExpOpt with VariablesExp with IfThenElseExp {
  ...
  override def var_new[T: Typ: Nul](init: Exp[T])(implicit pos: SourceContext): Var[T] = init match {
    case Def(Struct(tag, elems)) =>
      ...
      Variable(struct[Variable[T]](NestClassTag[Variable,T](tag), elems.map(p=>(p._1,var_new(p._2)(p._2.tp, p._2.nl, pos).e))))
    case _ =>
      super.var_new(init)
  }

  override def var_assign[T: Typ: Nul](lhs: Var[T], rhs: Exp[T])(implicit pos: SourceContext): Exp[Unit] = (lhs,rhs) match {
    case (Variable(Def(Struct(NestClassTag(tagL),elemsL: Seq[(String,Exp[Variable[Any]])]))), Def(Struct(tagR, elemsR))) =>
      ...
      for (((lk,lv), (rk,rv)) <- elemsL zip elemsR) {
        assert(lk == rk)
        var_assign(Variable(lv), rv)(rv.tp, rv.nl, pos)
      }
      Const(())
    case (Variable(Def(Struct(NestClassTag(tag), elems: Seq[(String,Exp[Variable[Any]])]))), Def(r)) => //TODO: keep this?
      for ((k,v) <- elems) {
        var_assign(Variable(v), field(r,k)(mtype(v.tp), ntype(v.nl), pos))(unwrap(v.tp), v.nl.asInstanceOf[Nul[Any]], pos)
      }
      Const(())
  ...
  override def readVar[T: Typ: Nul](v: Var[T])(implicit pos: SourceContext): Exp[T] = v match {
    case Variable(Def(Struct(NestClassTag(tag), elems: Seq[(String,Exp[Variable[Any]])]))) =>
      struct[T](tag, elems.map(p=>(p._1,readVar(Variable(p._2))(unwrap(p._2.tp), p._2.nl.asInstanceOf[Nul[Any]], pos))))
    case Variable(Def(Field(struct,idx))) =>
      field[T](struct, idx)
    case _ => super.readVar(v)
  }

  override def ifThenElse[T: Typ: Nul](cond: Rep[Boolean], a: Block[T], b: Block[T])(implicit pos: SourceContext) = (a,b) match {
    case (Block(Def(Struct(tagA,elemsA))), Block(Def(Struct(tagB, elemsB)))) =>
      assert(tagA == tagB)
      val elemsNew = for (((lk,lv), (rk,rv)) <- elemsA zip elemsB) yield {
        assert(lk == rk)
        lk -> ifThenElse(cond, Block(lv), Block(rv))(rv.tp, rv.nl, pos)
      }
...
trait StructFatExpOptCommon extends StructFatExp with StructExpOptCommon with IfThenElseFatExp {
  ...
  def phi[T: Typ: Nul](c: Exp[Boolean], a1: Block[Unit], a2: Exp[T], b1: Block[Unit], b2: Exp[T])(parent: Exp[Unit]): Exp[T] = if (a2 == b2) a2 else Phi(c,a1,Block(a2),b1,Block(b2))(parent)
  def phiB[T: Typ: Nul](c: Exp[Boolean], a1: Block[Unit], a2: Block[T], b1: Block[Unit], b2: Block[T])(parent: Exp[Unit]): Exp[T] = if (a2 == b2) a2.res else Phi(c,a1,a2,b1,b2)(parent) // FIXME: duplicate
  ...
  def deReify[T: Typ: Nul](a: Block[T]): (Block[Unit], Rep[T]) = a match { // take Reify(stms, e) and return Reify(stms, ()), e
    case Block(Def(Reify(x,es,u))) => (Block(Reify(Const(()), es, u)), x)
    case Block(x) => (Block(Const(())), x)
  }


  override def ifThenElse[T: Typ: Nul](cond: Rep[Boolean], a: Block[T], b: Block[T])(implicit pos: SourceContext) = (deReify(a),deReify(b)) match {
    case ((u, Def(Struct(tagA,elemsA))), (v, Def(Struct(tagB, elemsB)))) =>
      ...
      val elemsNew = for (((lk,lv), (rk,rv)) <- elemsA zip elemsB) yield {
        assert(lk == rk)
        lk -> phi(cond,u,lv,v,rv)(combinedResult)(mtype(lv.tp), ntype(lv.nl))
      }
      struct[T](tagA, elemsNew)
