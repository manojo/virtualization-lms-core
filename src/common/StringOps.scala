...
trait StringOps extends Variables with OverloadHack with PrimitiveOps {
  ...
  implicit def stringNul: Nul[String]

  def infix_+(s1: String, s2: Rep[Any])(implicit o: Overloaded1, pos: SourceContext) = string_plus(unit(s1), s2)
  def infix_+[T: Typ: Nul](s1: String, s2: Var[T])(implicit o: Overloaded2, pos: SourceContext) = string_plus(unit(s1), readVar(s2))
  def infix_+(s1: Rep[String], s2: Rep[Any])(implicit o: Overloaded1, pos: SourceContext) = string_plus(s1, s2)
  def infix_+[T: Typ: Nul](s1: Rep[String], s2: Var[T])(implicit o: Overloaded2, pos: SourceContext) = string_plus(s1, readVar(s2))
  ...
  def infix_+[T: Typ: Nul](s1: Var[String], s2: Var[T])(implicit o: Overloaded9, pos: SourceContext) = string_plus(readVar(s1), readVar(s2))
  ...
  def infix_+[T: Typ: Nul](s1: Var[T], s2: Rep[String])(implicit o: Overloaded12, pos: SourceContext) = string_plus(readVar(s1), s2)
  def infix_+[T: Typ: Nul](s1: Var[T], s2: Var[String])(implicit o: Overloaded13, pos: SourceContext) = string_plus(readVar(s1), readVar(s2))
  def infix_+[T: Typ: Nul](s1: Var[T], s2: String)(implicit o: Overloaded14, pos: SourceContext) = string_plus(readVar(s1), unit(s2))
...
trait StringOpsExp extends StringOps with BooleanOpsExp with VariablesExp {
  //implicit def arrayTyp[T: Typ: Nul]: Typ[Array[T]]
  implicit def stringTyp: Typ[String] = manifestTyp

  implicit def stringNul = new Nul[String] {
    def nullValue = unit("")
    def nlArguments = Nil
  }
