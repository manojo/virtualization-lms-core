...
trait SplitEffectsExpFat extends IfThenElseFatExp with WhileExp with PreviousIterationDummyExp { thisIR: BooleanOpsExp with EqualExpBridge =>
  ...
  override def reflectEffectInternal[A: Typ: Nul](x: Def[A], u: Summary)(implicit pos: SourceContext): Exp[A] = x match {
  ...
  def projectPureWithB[A: Typ: Nul](b: Block[A], s: List[Sym[Any]]): Block[A] = {
    (projectPureB(b), projectB(b,s)) match {
      case (Block(x), Block(Def(Reify(Const(()), u, es)))) => Block(Reify(x, u, es))
      case (a,_) => a
    }
  }
  def projectPureWithSimpleB[A: Typ: Nul](b: Block[A]): Block[A] = {
...
trait BaseGenSplitEffects extends BaseGenIfThenElseFat with GenericFatCodegen {
  ...
  override def reifyBlock[T: Typ: Nul](x: => Exp[T]): Block[T] = {
