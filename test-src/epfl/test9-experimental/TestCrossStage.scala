...
class TestCrossStage extends FileDiffSuite {
  ...
  trait DSL extends Functions with ArrayBufferOps with PrimitiveOps with OrderingOps with Variables with LiftVariables with IfThenElse with RangeOps with Print {
    def infix_toDouble(x: Rep[Int]): Rep[Double] = x.asInstanceOf[Rep[Double]]
    def test(x: Rep[Int]): Rep[Unit]

    implicit def funToRep[T:Typ,U:Typ](x:T=>U): Rep[T=>U]
    implicit def abToRep[T: Typ: Nul](x:ArrayBuffer[T]): Rep[ArrayBuffer[T]]
  }

  trait Impl extends DSL with StaticDataExp with FunctionsExp with ArrayBufferOpsExp with CoreOpsPkgExp with OrderingOpsExp with VariablesExp
    ...
    def abToRep[T: Typ: Nul](x:ArrayBuffer[T]): Rep[ArrayBuffer[T]] = staticData(x)

    override val verbosity = 2
    val codegen = new ScalaGenStaticData with ScalaGenFunctions with ScalaGenArrayBufferOps with ScalaGenPrimitiveOps with ScalaGenOrderingOps
