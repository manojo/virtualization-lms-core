...
trait MatcherNewProgB extends Util { this: PrimitiveOps with BooleanOps with StringOps with Functions with Equal with IfThenElse =>
  ...
  def interpret[A: Typ: Nul](xs: IO)(k: List[Rep[Char]=>IO] => Rep[A]): Rep[A] = xs match {
    case Nil => k(Nil)
    case ID(s,e)::rest =>
...
trait Util extends Base with LiftPrimitives with PrimitiveOps with Functions {
  ...
  def collectall(in: List[Rep[Any]]): Rep[Unit]
  def protect[A: Typ: Nul](x: Rep[A], in: List[Rep[Any]]): Rep[A]
}
...
class TestMatcherNew extends FileDiffSuite {
  ...
  trait DSL extends MatcherNewProg with PrimitiveOps with Functions with Equal with IfThenElse {
    def bare[T: Typ: Nul](x: Rep[Any], f: String => String): Rep[T]
  ...
  trait Impl extends DSL with RunTest with DFAOpsExp with GAOpsExp
    ...
    with CompileScala { q =>
      ...
      def protect[A: Typ: Nul](x: Exp[A], in: List[Rep[Any]]): Rep[A] = ResultA(x,in)
      def bare[T: Typ: Nul](x: Exp[Any], f: String => String): Exp[T] = Bare[T](x,f)
  ...
  def testCounter1 = withOutFileChecked(prefix+"counter1") {
    trait Prog extends DSL with ListOps with LiftPrimitives {
      ...
      def protect[A: Typ: Nul](a:Rep[A],b:Rep[Any]): Rep[A] = protect(a, Seq(b).toList)
  ...
  trait StreamHelpers extends DSL with ListOps with LiftAll with PrimitiveOps with BooleanOps with TupleOps with OrderingOps with StepperOps {
    ...
    def ptails[T: Typ: Nul](xs: Rep[List[T]]) = Prod[List[T]](xs, !list_isEmpty(_), list_tail(_))
    def plist[T: Typ: Nul](xs: Rep[List[T]]) = ptails(xs) map (list_head(_))

    def fcount[T: Typ: Nul] = Foreach[T,Double](0, c => s => s + 1)
    def fwhile[T: Typ: Nul](p: Rep[T] => Rep[Boolean]) = Foreach[T,Boolean](unit(true), c => s => if (s && p(c)) unit(true) else unit(false))
    def flast[T: Typ: Nul](s: Rep[T]) = Foreach[T,T](s, c => s => c)
    def fcollect[T: Typ: Nul] = Foreach[T,List[T]](List[T](), c => s => list_concat(s,List(c)))
