...
trait FWTransform1 extends BaseFatExp with EffectExp with IfThenElseFatExp with LoopsFatExp { self =>
  ...
  implicit def toAfter[A: Typ: Nul](x: Def[A]) = new { def atPhase(t: MyWorklistTransformer)(y: => Exp[A]) = transformAtPhase(x)(t)(y) }
  ...
  protected override implicit def toAtom[A: Typ: Nul](d: Def[A])(implicit pos: SourceContext): Exp[A] = { // override createDefinition instead?
    val in = syms(d)
    val actual = xform(in)
...
trait VectorExpTrans1 extends FWTransform1 with VectorExp with ArrayLoopsExp with ArrayMutationExp
  ...
  override def vapply[T: Typ: Nul](a: Rep[Vector[T]], x: Rep[Int]) = (a,x) match {
...
/*
  override def vlength[T: Typ: Nul](a: Rep[Vector[T]]) = a match {
  ...
  def vfromarray[A: Typ: Nul](x: Exp[Array[A]]): Exp[Vector[A]] = VectorFromArray(x)
  def vtoarray[A: Typ: Nul](x: Exp[Vector[A]]): Exp[Array[A]] = x match {
...
*/
  ...
  def vfromarray[A: Typ: Nul](x: Exp[Array[A]]): Exp[Vector[A]] = struct(ClassTag[Vector[A]]("Vector"), "data" -> x, "length" -> x.length)
  def vtoarray[A: Typ: Nul](x: Exp[Vector[A]]): Exp[Array[A]] = field[Array[A]](x, "data")

  override def vlength[T: Typ: Nul](a: Rep[Vector[T]]) = {
...
class TestForward1 extends FileDiffSuite {
  ...
  trait Impl extends DSL with VectorExpTrans1 with PrimitiveOpsExp with OrderingOpsExpOpt with BooleanOpsExp
