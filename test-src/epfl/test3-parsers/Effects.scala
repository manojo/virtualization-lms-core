...
trait Effects extends Base {
  ...
  implicit def effectfulTyp[A: Typ: Nul]: Typ[Effectful[A]]

  def noEffect: State
  def bindEffect[A: Typ: Nul](x: State, y: Rep[A]): State
  def reifyState[A: Typ: Nul](x: Rep[A], y: State): Rep[Effectful[A]]

  var context: State = _

  def reflectEffect[A: Typ: Nul](x: Rep[A]): Rep[A] = {
    context = bindEffect(context, x)
    x
  }

  def reifyEffects[A: Typ: Nul](block: => Rep[A]): Rep[Effectful[A]] = {
    val save = context
    context = noEffect
...
trait Control extends Effects with BaseExp {
  ...
  case class OrElse[A: Typ: Nul](x: List[Rep[Effectful[A]]]) extends Def[A]

//  def orElse[A](xs: List[Rep[Effectful[A]]]): Rep[A] = reflectEffect(OrElse(xs))
  def orElse[A: Typ: Nul](xs: List[Rep[Effectful[A]]]): Rep[A] = OrElse(xs)
  ...
  case class Reify[A: Typ: Nul](x: Rep[A], es: List[Rep[Any]]) extends Def[Effectful[A]]
  case class Pure[A: Typ: Nul](x: Rep[A]) extends Exp[Effectful[A]]

  def noEffect: State = Nil
  def bindEffect[A: Typ: Nul](x: State, y: Rep[A]): State = x:::List(y)
  def reifyState[A: Typ: Nul](x: Rep[A], y: State): Rep[Effectful[A]] = y match {
    case Nil => Pure(x)
    case _ => Reify(x, y)
  }
}
